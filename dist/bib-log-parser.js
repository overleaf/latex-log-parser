...
define(function() {
  var BibLogParser, LINE_SPLITTER_REGEX, MESSAGE_LEVELS, MULTILINE_WARNING_REGEX, SINGLELINE_WARNING_REGEX, consume;
  ...
  consume = function(logText, regex) {
    var fileName, fullMatch, index, iterationCount, lineNumber, match, message, newEntry, re, result, text;
    text = logText;
    result = [];
    re = regex;
    iterationCount = 0;
    while (match = re.exec(text)) {
      ...
      text = (match.input.slice(0, index)) + (match.input.slice(index + fullMatch.length + 1, match.input.length));
    }
    return [result, text];
  };
  (function() {
    this.parseBibtex = function() {
      ...
      ref = consume(this.text, MULTILINE_WARNING_REGEX), multiLineWarnings = ref[0], remainingText = ref[1];
      result.all = multiLineWarnings;
      result.warnings = multiLineWarnings;
      ref1 = consume(remainingText, SINGLELINE_WARNING_REGEX), singleLineWarnings = ref1[0], remainingText = ref1[1];
