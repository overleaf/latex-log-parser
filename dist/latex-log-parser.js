...
define(function() {
  var HBOX_WARNING_REGEX, LATEX_WARNING_REGEX, LINES_REGEX, LOG_WRAP_LIMIT, LatexParser, LogText, PACKAGE_REGEX, PACKAGE_WARNING_REGEX, PACKAGE_PROVIDES_REGEX, PACKAGE_PROVIDES_WARNING_REGEX, state;
  LOG_WRAP_LIMIT = 79;
  ...
  (function() {
      ...
      while ((this.currentLine = this.log.nextLine()) !== false) {
        if (this.state === state.NORMAL) {
          if (this.currentLineIsError()) {
            ...
            this.parseMultipleWarningLine(null, PACKAGE_REGEX, PACKAGE_WARNING_REGEX);
          } else {
    ...
    this.parseMultipleWarningLine = function(prefix_regex, error_type_regex, error_type_warning_regex) {
      var line, lineMatch, packageMatch, packageName, prefixRegex, raw_message, warningMatch, warning_lines;
      warningMatch = this.currentLine.match(error_type_warning_regex);
      if (!warningMatch) {
        return;
      }
      ...
      line = lineMatch ? parseInt(lineMatch[1], 10) : null;
      packageMatch = this.currentLine.match(error_type_regex);
      packageName = packageMatch[1];
      trimRegex = new RegExp('(?:\\(' + packageName + '\\))*[\\s]*(.*)', 'i');
      while (!!(this.currentLine = this.log.nextLine())) {
        lineMatch = this.currentLine.match(LINES_REGEX);
        line = lineMatch ? parseInt(lineMatch[1], 10) : line;
        warningMatch = this.currentLine.match(trimRegex);
